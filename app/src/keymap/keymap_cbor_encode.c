		...
		zcbor_state_t *state, const struct key *input)
{
	...
	&& (zcbor_list_start_encode(state, 256) && ((zcbor_multi_encode_minmax(1, 256, &(*input)._key_parameters_tstr_count, (zcbor_encoder_t *)zcbor_tstr_encode, state, (&(*input)._key_parameters_tstr), sizeof(struct zcbor_string))) || (zcbor_list_map_end_force_encode(state), false)) && zcbor_list_end_encode(state, 256)))) || (zcbor_list_map_end_force_encode(state), false)) && zcbor_map_end_encode(state, 2))));
		...
		zcbor_state_t *state, const struct layer *input)
{
	...
	&& (zcbor_list_start_encode(state, 256) && ((zcbor_multi_encode_minmax(1, 256, &(*input)._layer_keys__key_count, (zcbor_encoder_t *)encode_key, state, (&(*input)._layer_keys__key), sizeof(struct key))) || (zcbor_list_map_end_force_encode(state), false)) && zcbor_list_end_encode(state, 256)))) || (zcbor_list_map_end_force_encode(state), false)) && zcbor_map_end_encode(state, 2))));
		...
		zcbor_state_t *state, const struct keymap *input)
{
	...
	&& (zcbor_list_start_encode(state, 256) && ((zcbor_multi_encode_minmax(1, 256, &(*input)._keymap_layers__layer_count, (zcbor_encoder_t *)encode_layer, state, (&(*input)._keymap_layers__layer), sizeof(struct layer))) || (zcbor_list_map_end_force_encode(state), false)) && zcbor_list_end_encode(state, 256)))
	&& (((zcbor_tstr_encode(state, ((tmp_str.value = (uint8_t *)"supported_behaviors", tmp_str.len = sizeof("supported_behaviors") - 1, &tmp_str)))))
	&& (zcbor_list_start_encode(state, 256) && ((zcbor_multi_encode_minmax(1, 256, &(*input)._keymap_supported_behaviors_tstr_count, (zcbor_encoder_t *)zcbor_tstr_encode, state, (&(*input)._keymap_supported_behaviors_tstr), sizeof(struct zcbor_string))) || (zcbor_list_map_end_force_encode(state), false)) && zcbor_list_end_encode(state, 256)))) || (zcbor_list_map_end_force_encode(state), false)) && zcbor_map_end_encode(state, 3))));
